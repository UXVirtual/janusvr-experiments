...
void main(void) {
  ...
  float move_bush = sin(iGlobalTime*3.0 + gl_Vertex.y/10.0 + gl_Vertex.x/10.0) * (gl_Vertex.x+gl_Vertex.y) * 0.01;
  gl_Position = gl_ModelViewProjectionMatrix * (gl_Vertex + vec4(move_bush, move_bush, move_bush, 0));
